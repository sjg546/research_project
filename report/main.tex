...
\begin{abstract}
  ...
  and individual matches having enormous monetary ramifications' and impacts
  for player legacy.
  ...
  contrasting these different approaches and their impact on prediction accuracy,
  log loss and calibration was investigated in this report. Investigating building
  an Elo model for a players
  performance on given surfaces and then combining that with the base Elo model was done to
  investigate improvements to overall model performance.
...
ranging from simplistic linear/logistic models to complex Elo rating based models.
This report investigated a subset of these models and identified
which of these performed the best as measured by a combination of metrics,
and what future work will be conducted to
improve the overall performance of these models. Log loss, which measures how close
...
Calibration for each of the models was also calculated which represents the whether
the model over or underestimates the highest ranked players wins.
\noindent \hrulefill
...
The Elo Rating Algorithm is a commonly used method of ranking players in competitive "zero sum"
games, meaning there is no possibility for a tie. Some examples of this include official Chess rankings and online Player vs Player (PvP)
...
t'th match and $E_i(t)$ is the Elo rating of player i at match t.
...
optimal values. Optimising these hyperparameters was investigated in the paper as
these optimal values have a significant impact on the models overall performance.
...
the test set to evaluate performance.
...
the logistic model, the parameters will be calculated utilising the R studio
...
which don't have all information required for the model to run. To align with the Tennis
Data UK dataset which will be explained below, only the data from the years 2001 to 2024
will be considered.
\\
\\
...
the search space needed to be reduced when compared to the K Factor model.
...
combining these models was taking the calculated probability of player 1 winning, $\pi_1$,
from each models and combining them to generate a new probability $\pi_{1,c}$ based
...
\end{table}
From the results above it can be observed that as the K value increases up to 30, the log
loss decreases. After this the accuracy remains stable however the corresponding log loss
begins to increase. As this model does not consider the past performance of the players when updating their corresponding
...
$\approx 0.002$. When looking at the Surface Elo model that hasn't been combined with an
...
model can't be used until after a match is played making it unsuitable as a solution for
...
\begin{figure}[H]
  ...
  \caption{K Factor, Five Thirty Eight and Surface Combined Elos for Roger Federers career
    showing the difference in ratings change across the various models}
...
makes the least correct predictions. The accuracy of the logistic model is
...
near optimal values. Due to the complexity of implementation and minimal potential for metric
...
therefore the impact of any suboptimal hyperparameters would be reduced. In the future work
...
outperforms all the models in both accuracy and log loss. It does have a worse calibration
...
certain odd aggregation websites. There are issues with building and using a system like
