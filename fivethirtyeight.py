    ...
    def run_metrics(self,df:pd.DataFrame,delta,nu,sigma):
        ...
        for index, row in df.iterrows():
           if(row[f"prob_{delta}_{nu}_{sigma}"] > 0.5 ):
               df.loc[index,"combined_log_loss"] = self.test_ll(1.0,row[f"prob_{delta}_{nu}_{sigma}"])

           else:
               df.loc[index,"combined_log_loss"] = self.test_ll(0.0,row[f"loser_prob_{delta}_{nu}_{sigma}"])
